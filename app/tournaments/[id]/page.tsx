...
function TournamentLiveLeaderboard({
  ...
  // Rounds that are officially settled on-chain (winnerDirection set by admin)
  const confirmedRoundSet = useMemo(
    () => new Set(tournament.rounds.filter((r) => r.winnerDirection !== 0).map((r) => r.roundNumber)),
    [tournament.rounds],
  );
    ...
    >
      ...
      {leaderboard.length > 0 && (
        <div className="flex items-center gap-1 mb-2 px-2">
          ...
          {sortedRounds.map((r, rIdx) => {
            const isConfirmed = confirmedRoundSet.has(r.roundNumber);
            const winDir = isConfirmed ? roundWinners.get(r.roundNumber) : undefined;
        ...
        {([DIRECTION.UP, DIRECTION.DOWN] as const).map((dir) => {
          ...
          return (
            <button
              ...
              disabled={!isLive || userParticipated}
              ...
              style={
                isSelected
                  ...
                  : {
                      ...
                      opacity: !isLive || userParticipated ? 0.5 : 1,
                    }
              }
            >
              {isUp ? "▲ UP" : "▼ DOWN"}
...
function ClaimPanel({
  ...
  const joinEventsQuery = useRoundJoinEvents(round.roundNumber);
  const { balance, refetch: refetchBalance } = useUSDTBalance(account?.address);
  ...
  const userJoinEvent = useMemo(
    () =>
      (joinEventsQuery.data ?? []).find(
        (e) => e.player.toLowerCase() === (account?.address ?? "").toLowerCase(),
      ),
    [joinEventsQuery.data, account?.address],
  );

  if (!userJoinEvent || round.winnerDirection === 0 || (!round.isSettled && round.status !== "ended")) {
    return null;
  }

  const isWinner = userJoinEvent.direction === round.winnerDirection;
  ...
  async function handleClaim() {
    ...
    try {
      if (isCancelled) {
        await claimRefund(round.roundObjectId);
        setMsg("Refund claimed!");
      } else {
        await claimReward(round.sessionId, round.roundObjectId);
        setMsg(isWinner ? "Reward claimed!" : "Entry returned!");
      }
      refetchBalance();
    } catch (error) {
  ...
  return (
    <div
      ...
      style={{
        backgroundColor: isWinner ? "rgba(13,242,128,0.08)" : "rgba(239,68,68,0.06)",
        border: `1px solid ${isWinner ? "rgba(13,242,128,0.2)" : "rgba(239,68,68,0.15)"}`,
      }}
    >
      <p className="text-xs font-bold text-slate-300 mb-3">
        Round {round.roundNumber}: You predicted{" "}
        <strong style={{ color: userJoinEvent.direction === DIRECTION.UP ? "#22c55e" : "#ef4444" }}>
          {formatDirection(userJoinEvent.direction)}
        </strong>
        {" "}— {isWinner ? <span style={{ color: "#0df280" }}>Winner!</span> : <span style={{ color: "#ef4444" }}>Lost</span>}
      </p>
      <button
        type="button"
        onClick={handleClaim}
        ...
        style={{
          backgroundColor: isWinner ? "#0df280" : "rgba(255,255,255,0.07)",
          color: isWinner ? "#0a0a0a" : "#94a3b8",
      ...
      {msg && <p className="mt-2 text-[11px] font-bold text-slate-300">{msg}</p>}
    </div>
  );
}
      ...
      <main className="relative z-10 max-w-7xl mx-auto px-6 pt-24 pb-20">
            ...
            <div className="flex items-center gap-4 md:gap-6 flex-wrap">
              ...
              <div className="text-right">
                ...
                <p className="text-2xl font-black" style={{ color: "#0df280" }}>
                  {formatUSDT(tournament.totalPrizePoolRaw)} USDT
                </p>
              </div>
              {!account && (
                <ConnectButton />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            {/* Claim panels for ended rounds */}
            {account && tournament.rounds.filter(r => r.status === "ended" || r.isSettled).length > 0 && (
              <div
              ...
              >
                ...
                {tournament.rounds.map((r) => (
                  <ClaimPanel key={r.sessionId} round={r} account={account} />
                ))}
              </div>
            )}
          </div>
            ...
            {displayRound && (
              ...
              >
                ...
                {[
                  ...
                  { label: "Participants", value: displayRound.totalParticipants },
                  { label: "Pool", value: `${formatUSDT(displayRound.totalPoolRaw)} USDT` },
                ].map((item) => (
                  <div
                    key={item.label}
        ...
        {tournament.rounds.some((r) => r.status === "ended" || r.isSettled) && (
          <div className="mb-8">
            ...
            <div className="space-y-2">
              {(() => {
                // Sort all rounds by startTime to assign sequential indices (R1, R2, …)
                ...
                const mostRecentId = doneRounds[0]?.sessionId;
