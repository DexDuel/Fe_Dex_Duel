...
const VALID_STATUSES = new Set<string>(["PENDING", "ACTIVE", "ENDED", "SETTLED"]);
...
export async function GET(req: NextRequest) {
    try {
        ...
        const cursor = sp.get("cursor");     // id (cuid) of the last seen round
        ...
        if (status && VALID_STATUSES.has(status)) {
            where.status = status as RoundStatus;
        }

        if (q) {
            // Try to parse as a roundId number first; fall back to coinSymbol substring
            const asNumber = (() => { try { return BigInt(q); } catch { return null; } })();
            where.OR = asNumber !== null
                ? [
                    { roundId: asNumber },
